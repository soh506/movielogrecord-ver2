...
import { fetchWithAuth, fetchCurrentUser, ApiError } from '@/lib/api';
...
function MovieDetailPage() {
    ...
    try {
      ...
      setDeleteError(
        e instanceof ApiError && e.status === 400
          ? 'ログが残っています。先にすべてのログを削除してから映画を削除してください。'
          : '削除に失敗しました'
      );
  ...
  async function handleAddLog(e: React.FormEvent) {
    ...
    try {
      ...
      setLogError(e instanceof ApiError && e.status === 403 ? '自分のログのみ保存できます' : 'ログの保存に失敗しました');
    }
  }
  ...
  async function handleEditLog(e: React.FormEvent, logId: number) {
    ...
    try {
      ...
      setLogError(e instanceof ApiError && e.status === 403 ? '自分のログのみ編集できます' : 'ログの更新に失敗しました');
    }
  }
  ...
  async function handleDeleteLog(logId: number) {
    ...
    try {
      ...
      setLogError(e instanceof ApiError && e.status === 403 ? '自分のログのみ削除できます' : 'ログの削除に失敗しました');
    }
  }
  ...
  if (loadError) return (
    <main className="min-h-screen bg-gray-50 flex items-center justify-center">
      <div className="text-center">
        <p className="text-sm text-red-600 mb-4">映画情報の取得に失敗しました</p>
        <Link href="/" className="text-sm text-blue-600 hover:underline">一覧に戻る</Link>
      </div>
    </main>
  );

  if (!movie) return <main className="min-h-screen bg-gray-50"><Spinner /></main>;
    ...
    <main className="min-h-screen bg-gray-50">
      <div className="max-w-xl mx-auto py-12 px-4">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          {logError && <p className="text-sm text-red-600 mb-3">{logError}</p>}
